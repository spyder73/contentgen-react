...
describe('IdeaInputForm modal attachment flow', () => {
  ...
  it('starts text-only when template has no start-time initial asset requirements', async () => {
    const onStart = jest.fn<Promise<void>, any[]>(async () => undefined);
    const templatesWithoutAttach: PipelineTemplate[] = [
      {
        ...templates[0],
    ...
    render(<IdeaInputForm templates={templatesWithoutAttach} onStart={onStart} />);

    expect(screen.queryByText('Seed Reference Image')).not.toBeInTheDocument();
    fireEvent.change(screen.getByPlaceholderText('Describe your video idea...'), {
      target: { value: 'Generate without a seed reference.' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Generate' }));

    await waitFor(() => expect(onStart).toHaveBeenCalledTimes(1));
    expect(onStart.mock.calls[0][3]).toEqual([]);
  });

  it('does not render a dedicated seed picker for checkpoint-scoped image requirements', async () => {
    const onStart = jest.fn<Promise<void>, any[]>(async () => undefined);
    const referencedTemplate: PipelineTemplate[] = [
      {
        ...templates[0],
        id: 'referenced-video-idea-generator',
        checkpoints: [
          {
            ...templates[0].checkpoints[0],
            id: 'character-seed',
    ...
    render(<IdeaInputForm templates={referencedTemplate} onStart={onStart} />);

    expect(screen.queryByText('Seed Reference Image')).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Select Seed Reference' })).not.toBeInTheDocument();
    expect(screen.queryByText(/Missing Required Checkpoint Assets/i)).not.toBeInTheDocument();

    fireEvent.change(screen.getByPlaceholderText('Describe your video idea...'), {
      target: { value: 'Generate the referenced pipeline from text only.' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Generate' }));
  ...
  it('serializes optional image attachments through the run attachment picker', async () => {
    const onStart = jest.fn<Promise<void>, any[]>(async () => undefined);
    const templatesWithoutAttach: PipelineTemplate[] = [
      {
        ...templates[0],
    ...
    mockedApi.listMediaLibrary.mockResolvedValue([
      {
        ...
        type: 'image',
        name: 'Seed Frame',
        url: 'https://cdn.example.com/seed-frame.png',
        source: 'manual_upload',
        mime_type: 'image/png',
      },
    ]);
    ...
    fireEvent.click(screen.getByRole('button', { name: /Seed Frame/i }));
    fireEvent.click(screen.getByRole('button', { name: 'Add Selected (1)' }));

    fireEvent.change(screen.getByPlaceholderText('Describe your video idea...'), {
      target: { value: 'Generate with a seed frame.' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Generate' }));
  ...
  it('keeps Browse Files tab active while library data refreshes in manage mode', async () => {
    mockedApi.listMediaLibrary.mockResolvedValue([
      {
        id: 'media-image-1',
        media_id: 'media-image-1',
    ...
    expect(screen.queryByText('Upload files to your media library.')).not.toBeInTheDocument();
    expect(mockedApi.listMediaLibrary).toHaveBeenCalledTimes(1);
  });
    ...
    const templatesWithRequired: PipelineTemplate[] = [
      {
        ...
        checkpoints: [
          {
            ...templates[0].checkpoints[0],
            required_assets: [{ key: 'req-image', type: 'image', source: 'initial' }],
          },
        ],
      },
    ];
  ...
  it('does not show start-time requirement warnings for paused-step asset requirements', async () => {
    const onStart = jest.fn<Promise<void>, any[]>(async () => undefined);
    const templatesWithPausedRequirement: PipelineTemplate[] = [
      {
        ...templates[0],
        checkpoints: [
          {
    ...
    render(<IdeaInputForm templates={templatesWithPausedRequirement} onStart={onStart} />);

    expect(screen.queryByText(/Missing Required Checkpoint Assets/i)).not.toBeInTheDocument();

    fireEvent.change(screen.getByPlaceholderText('Describe your video idea...'), {
      target: { value: 'Pause later for the asset instead of blocking start.' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Generate' }));
